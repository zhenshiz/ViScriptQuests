...
public abstract class IQuestReward implements ILDLRegister<IQuestReward, Supplier<IQuestReward>>, IPersistedSerializable, IConfigurable {
    ...
    public Configurator createConfigurator() {
        ...
        ViScriptQuestsRegistries.QUEST_REWARD.forEach(questReward -> {
            IQuestReward iQuestReward = questReward.value().get();
            arr.put(iQuestReward.getType(), iQuestReward.getIcon());
        });
        ...
        SearchComponentConfigurator<String> typeConfigurator = UIElementUtil.createTaskTypeSearchComponentConfigurator("viscript_quests.reward.type", arr, this::getUiType, this::setUiType);
        typeConfigurator.searchComponent.addEventListener(UIEvents.LAYOUT_CHANGED, event -> {
            configuratorGroup.removeAllConfigurators();
            ViScriptQuestsRegistries.QUEST_REWARD.forEach(questReward -> {
                IQuestReward iQuestReward = questReward.value().get();
                if (iQuestReward.getType().equals(uiType)) {
                    configuratorGroup.addConfigurators(iQuestReward.createDirectConfigurator());
                }
            });
        });
    ...
    // 获取奖励描述 (给 UI 用)
    abstract public Component getDescription();

    // 给玩家发放奖励
    abstract public void grant(ServerPlayer player);
